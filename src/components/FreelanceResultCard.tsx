...
function HeroKpi({ netMensuel, netAnnuel, label }: { netMensuel: number; netAnnuel: number; label: string }) {
  return (
    <div className="px-5 pt-5 pb-4 text-center border-b border-gray-100 dark:border-white/5">
      <p className="text-[10px] font-display font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1.5">{label}</p>
      <p className="font-mono text-4xl font-bold tabular-nums text-gray-900 dark:text-white">{fmt(netMensuel)}</p>
      <p className="font-mono text-sm tabular-nums text-gray-400 dark:text-gray-500 mt-1">
        {fmt(netAnnuel)}<span className="font-sans text-xs"> /an</span>
      </p>
    </div>
  );
}
...
function MicroCard({ results, inputs }: { results: MicroResults; inputs: FreelanceInputs }) {
  ...
  return (
    <div className="rounded-xl border border-indigo-100 dark:border-indigo-900/40 bg-white dark:bg-[#0F0F1C] shadow-sm overflow-hidden">
      <div className="bg-indigo-50 dark:bg-indigo-950/20 border-b border-indigo-100 dark:border-indigo-900/40 px-4 py-2">
        <p className="text-xs text-indigo-500 dark:text-indigo-400 font-medium">
          Micro-entrepreneur — {inputs.joursAnnuel} j/an
        </p>
      </div>
      <HeroKpi netMensuel={results.netMensuel} netAnnuel={results.netAnnuel} label="Net mensuel après IR" />
      <div className="p-4 grid grid-cols-2 gap-4">
        <div className="space-y-0.5">
          <p className="text-xs text-gray-400 dark:text-gray-500">CA annuel brut</p>
          <p className="font-mono font-semibold text-gray-700 dark:text-gray-300 tabular-nums text-sm">{fmt(results.caAnnuel)}</p>
        </div>
        <div className="space-y-0.5">
          ...
          <p className="font-mono font-semibold text-gray-700 dark:text-gray-300 tabular-nums text-sm">
...
function SasuCard({ results, inputs }: { results: SasuResults; inputs: FreelanceInputs }) {
  return (
    <div className="rounded-xl border border-indigo-100 dark:border-indigo-900/40 bg-white dark:bg-[#0F0F1C] shadow-sm overflow-hidden">
      <div className="bg-indigo-50 dark:bg-indigo-950/20 border-b border-indigo-100 dark:border-indigo-900/40 px-4 py-2 flex items-center justify-between">
        <p className="text-xs text-indigo-500 dark:text-indigo-400 font-medium">
          SASU — Président assimilé salarié
        </p>
        <p className="text-xs text-indigo-400 dark:text-indigo-500">{inputs.joursAnnuel} j/an</p>
      </div>
      <HeroKpi netMensuel={results.netMensuel} netAnnuel={results.netAnnuel} label="Net mensuel disponible" />
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-0.5">
            ...
            <p className="font-mono font-semibold text-gray-500 dark:text-gray-500 tabular-nums text-sm">
              {fmt(results.resultatAvantIS)}
            </p>
          </div>
        </div>

        {/* Rémunération + Dividendes côte à côte */}
        <div className="grid grid-cols-2 gap-3 items-start">
          {/* Salaire */}
          <div className="rounded-lg bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/40 p-3 space-y-2.5">
            <p className="text-xs font-display font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wide">Rémunération</p>
            <div className="space-y-1">
              ...
              <div className="pt-1 border-t border-blue-100 dark:border-blue-900/40 grid grid-cols-2 gap-1.5">
                <span className="text-[11px] font-semibold text-gray-700 dark:text-gray-300">Coût total SASU</span>
                <span className="text-[11px] font-mono font-semibold text-gray-900 dark:text-gray-100 tabular-nums text-right">
                  {fmt(results.salaireBrut + results.chargesPatronales)}
                </span>
              </div>
            </div>
            <div className="border-t border-blue-200 dark:border-blue-800/40" />
            <div className="space-y-1">
              <p className="text-[10px] font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide">
              ...
              <div className="pt-1 border-t border-blue-100 dark:border-blue-900/40 grid grid-cols-2 gap-1.5">
                ...
                <span className="text-xs font-mono font-semibold text-gray-900 dark:text-gray-100 tabular-nums text-right">{fmt(results.salaireNetApresIR)}</span>
              </div>
            </div>
          </div>

          {/* IS + Dividendes */}
          <div className="rounded-lg bg-violet-50 dark:bg-violet-950/20 border border-violet-100 dark:border-violet-900/40 p-3 space-y-1.5">
            <p className="text-xs font-display font-semibold text-violet-600 dark:text-violet-400 uppercase tracking-wide">Dividendes</p>
            <div className="grid grid-cols-2 gap-2 text-[11px] text-gray-500 dark:text-gray-400">
              ...
              <span className="font-mono tabular-nums text-right font-medium text-gray-600 dark:text-gray-300">{fmt(results.resultatNetIS)}</span>
              <span>− Flat tax dividendes (30%)</span>
              <span className="tabular-nums text-right text-red-400">−{fmt(results.flatTaxDividendes)}</span>
            </div>
            <div className="pt-1 border-t border-violet-100 dark:border-violet-900/40 grid grid-cols-2 gap-2">
              <span className="text-xs text-gray-600 dark:text-gray-300">Dividendes nets</span>
              <span className="text-xs font-mono font-semibold text-gray-900 dark:text-gray-100 tabular-nums text-right">{fmt(results.dividendesNet)}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="rounded-xl border border-indigo-100 dark:border-indigo-900/40 bg-white dark:bg-[#0F0F1C] shadow-sm overflow-hidden">
      ...
      <div className="p-4 space-y-4">
        ...
        {/* Rémunération TNS + Dividendes côte à côte */}
        <div className="grid grid-cols-2 gap-3 items-start">
          {/* Rémunération TNS */}
          <div className="rounded-lg bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/40 p-3 space-y-2.5">
            <p className="text-xs font-display font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wide">Rémunération TNS</p>
            <div className="space-y-1">
              <p className="text-[10px] font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide">
                Coût pour l'EURL
            ...
            <div className="space-y-1">
              ...
              <div className="grid grid-cols-2 gap-1.5 text-[11px] text-gray-500 dark:text-gray-400">
                ...
                <span className="font-mono tabular-nums text-right font-medium text-gray-600 dark:text-gray-300">{fmt(results.remunerationNetteAvantIR)}</span>
                <span>− PAS (retenue à la source)</span>
              ...
              <div className="pt-1 border-t border-blue-100 dark:border-blue-900/40 grid grid-cols-2 gap-1.5">
                <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">Net après IR</span>
                <span className="text-xs font-mono font-semibold text-gray-900 dark:text-gray-100 tabular-nums text-right">{fmt(results.remunerationNetteApresIR)}</span>
              </div>
            </div>
          </div>
          ...
          {/* IS + Dividendes */}
          <div className="rounded-lg bg-violet-50 dark:bg-violet-950/20 border border-violet-100 dark:border-violet-900/40 p-3 space-y-1.5">
            <p className="text-xs font-display font-semibold text-violet-600 dark:text-violet-400 uppercase tracking-wide inline-flex items-center gap-1">
              Dividendes
...
export default function FreelanceResultCard({ results, inputs }: FreelanceResultCardProps) {
  if (!results || !hasData(inputs)) {
    return (
      <div className="rounded-xl border border-indigo-100 dark:border-indigo-900/40 bg-indigo-50/50 dark:bg-indigo-950/10 p-6 text-center text-indigo-300 dark:text-indigo-700/60 text-sm">
        Saisissez un TJM
      </div>
    );
  }
